...
execute at @p[distance=..6,gamemode=!spectator] if block ~ ~-1 ~ #scorched:activates_desert_pyramid_spawner at @s unless score @s scorched_count matches 0.. run function scorched:structure/desert/spawner/anim